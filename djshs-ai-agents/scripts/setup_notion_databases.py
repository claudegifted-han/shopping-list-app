#!/usr/bin/env python3
"""
DJSHS AI Agent System - Notion 데이터베이스 자동 생성 스크립트
...
class NotionDatabaseSetup:
    ...
    def setup_all_databases(self, parent_page_id: str) -> Dict[str, str]:
        """모든 데이터베이스 생성"""
        print("\n" + "="*60)
        print("🚀 DJSHS AI Agent System - Notion 데이터베이스 설정")
        print("="*60 + "\n")
    ...
    def generate_env_file(self, output_path: str = ".env.notion") -> None:
        """환경변수 파일 생성"""
        env_content = f"""# DJSHS AI Agent System - Notion 설정
...
def main():
    parser = argparse.ArgumentParser(
        description="DJSHS AI Agent System - Notion 데이터베이스 자동 설정"
    )
    parser.add_argument(
        "--api-key", "-k",
