...
- **파일 경로가 있으면** 새 심사를 시작한다. 과제 ID가 함께 오면 그것을 쓰고, 없으면 파일 이름에서 찾는다(예: `창의-07`, `물03`). 그래도 없으면 사용자에게 한 번 묻는다.
  - 채팅에 첨부한 파일(Claude Code 웹)은 `@"…/uploads/…"`처럼 들어온다. `@`와 따옴표를 떼고 그 경로를 쓴다. 파일 이름 앞에 붙은 임의의 글자(예: `7b3c05c9-`)는 과제 ID가 아니다.
- **과제 ID만 있으면** 이미 있는 카드를 이어서 진행한다(중간에 끊겼을 때). 바로 반복 단계로 간다.
  - 그 ID의 카드가 없으면 새 심사로 본다. 이 대화에 첨부된 제안서가 있으면 그것으로 시작하고, 없으면 제안서 파일을 한 번 묻는다.
